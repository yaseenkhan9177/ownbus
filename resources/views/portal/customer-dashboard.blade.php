...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl">
            ...
            <p class="text-3xl font-black text-yellow-400 mt-2">{{ number_format($loyaltyPoints) }}</p>
        ...
        <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl">
            ...
            <p class="text-3xl font-black text-white mt-2">AED {{ number_format($totalSpent, 0) }}</p>
        ...
        <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl">
            ...
            <p class="text-3xl font-black text-blue-400 mt-2">{{ $activeBookings->count() }}</p>
    ...
    <div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">
                        {{ $rental->start_date?->format('d M Y') }} — {{ $rental->end_date?->format('d M Y') ?? 'Ongoing' }}
                    </p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                ...
                <span class="text-sm font-black text-white">AED {{ number_format($rental->final_amount ?? $rental->grand_total, 0) }}</span>
            </div>
        </div>
        @empty
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-slate-800/50">
                    ...
                    <tr class="hover:bg-slate-800/30 transition">
                        ...
                        <td class="px-6 py-4 text-xs text-slate-400">{{ $rental->start_date?->format('d M') }} – {{ $rental->end_date?->format('d M y') }}</td>
