...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-slate-900 dark:text-white mt-1">UPDATE_PERSONNEL</h1>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-1">Modifying Profile for {{ $driver->name }}</p>
        </div>
        <a href="{{ route('company.drivers.show', $driver) }}" class="text-[10px] font-black text-slate-400 hover:text-slate-600 uppercase tracking-widest transition-all">
            <i class="bi bi-arrow-left mr-1"></i> ABORT_MODIFICATION
        </a>
    </div>

    <form action="{{ route('company.drivers.update', $driver) }}" method="POST" class="space-y-6" x-data="{ submitting: false }" @submit="submitting = true">
        @csrf
        @method('PATCH')
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
            <div class="px-6 py-4 border-b border-slate-50 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50 flex items-center gap-3">
                ...
                <h2 class="text-xs font-black text-slate-900 dark:text-white uppercase tracking-widest">PROTOCOL_STATE</h2>
            </div>
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">CURRENT_STATUS</label>
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea name="address" rows="2" required
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">{{ old('address', $driver->address) }}</textarea>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="national_id" value="{{ old('national_id', $driver->national_id) }}" required
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="license_type" required
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all appearance-none">
                        <option value="light" {{ old('license_type', $driver->license_type) == 'light' ? 'selected' : '' }}>LIGHT_VEHICLE</option>
                        <option value="heavy" {{ old('license_type', $driver->license_type) == 'heavy' ? 'selected' : '' }}>HEAVY_VEHICLE</option>
                        <option value="bus" {{ old('license_type', $driver->license_type) == 'bus' ? 'selected' : '' }}>BUS_AUTHORIZATION</option>
                    </select>
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">LICENSE_EXPIRY_DATE</label>
                    <input type="date" name="license_expiry_date" value="{{ old('license_expiry_date', $driver->license_expiry_date->format('Y-m-d')) }}" required
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="date" name="hire_date" value="{{ old('hire_date', $driver->hire_date->format('Y-m-d')) }}" required
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" step="0.01" name="salary" value="{{ old('salary', $driver->salary) }}"
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" step="0.01" name="commission_rate" value="{{ old('commission_rate', $driver->commission_rate) }}"
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" name="emergency_contact_name" value="{{ old('emergency_contact_name', $driver->emergency_contact_name) }}" required
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="emergency_contact_phone" value="{{ old('emergency_contact_phone', $driver->emergency_contact_phone) }}" required
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea name="notes" rows="3"
                        class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-3 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all">{{ old('notes', $driver->notes) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-4 pb-12">
            <button type="submit" x-bind:disabled="submitting" x-text="submitting ? 'UPDATING...' : 'SAVE_MODIFICATIONS'" class="bg-blue-600 text-white px-8 py-4 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-700 hover:scale-105 transition-all shadow-lg shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100">
                SAVE_MODIFICATIONS
            </button>
        </div>
    </form>
